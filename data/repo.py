...
class Repo:
    ...
    def _init_db(self):
        with self._conn() as con:
            ...
            """)
            # для теста — добавим пару новостей, если база пустая
            cur = con.execute("SELECT COUNT(*) FROM news")
            if cur.fetchone()[0] == 0:
                now = datetime.utcnow().isoformat()
                con.execute("INSERT INTO news(title, text, created_at) VALUES (?, ?, ?)",
    ...
    def list_pending(self) -> list[News]:
        with self._conn() as con:
            rows = con.execute(
                "SELECT id, title, text, status, model_score FROM news WHERE status='pending' ORDER BY id DESC"
            ).fetchall()
        return [News(*r) for r in rows]

    def get_threshold(self) -> float:
        with self._conn() as con:
            v = con.execute("SELECT value FROM settings WHERE key='threshold'").fetchone()[0]
        return float(v)

    def set_threshold(self, value: float):
        with self._conn() as con:
            con.execute("UPDATE settings SET value=? WHERE key='threshold'", (str(value),))

    def update_model_score(self, news_id: int, score: float):
        with self._conn() as con:
            con.execute("UPDATE news SET model_score=? WHERE id=?", (score, news_id))
